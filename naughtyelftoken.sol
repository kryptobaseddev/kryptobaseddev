...
Telegram: 
http://t.me/naughtyelftoken

Website:
http://www.naughtyelf.fun
...
contract PresentDistributor is IPresentDistributor {
...
contract NaughtyElf is ERC20, Ownable {
    ...
    uint256 public maxTx = 10 * TOTAL_SUPPLY / 1000 * (DECIMALS);               // 0.5%  of total supply, 5 M tokens
    uint256 public swapTokensAtAmount = 5 * TOTAL_SUPPLY / 1000 * (DECIMALS);   // 0.05% of total supply, 500 M tokens
    uint256 public maxWallet = 20 * TOTAL_SUPPLY / 1000 * (DECIMALS);          // 2.0%  of total supply, 20  M tokens  
    ...
    mapping (address => uint256) public naughtyTime;
    uint256 public noGiftsTime;
    ...
    event SetFees(uint256 indexed PresentRewardsFee, uint256 indexed marketingFee, uint256 indexed liquidityFee);
    ...
    event UpdatePresentDistributor(address indexed newAddress, address indexed oldAddress);
    ...
    constructor() ERC20("NaughtyElf", "NELF") {
        ...
        DaddyElf = new PresentDistributor(address(_uniswapV2Router), owner());
        ...
        excludeFromDividends(address(DaddyElf), true);
    ...
    function initiateAntiBot(uint256 _antiBotDuration) public onlyOwner{
        require(!tradingIsEnabled && !hasLaunched, "Project already launched.");
    ...
    function launch(uint256 _nAntiBotBlocks,uint256 _tradeCooldown) public onlyOwner{
        ...
        require(_nAntiBotBlocks < 5 && _tradeCooldown <= 10, "Chill.");
    ...
    function updatePresentDistributor(address newAddress) public onlyOwner {
        require(newAddress != address(DaddyElf), "DaddyElf: The dividend distributor already has that address");

        PresentDistributor newDaddyElf = PresentDistributor(payable(newAddress));

        require(newDaddyElf._owner() == address(this), "DaddyElf: The new dividend distributor must be owned by the Test token contract");

        excludeFromDividends(address(newDaddyElf), true);
        excludeFromDividends(address(this), true);
        excludeFromDividends(address(uniswapV2Router), true);

        emit UpdatePresentDistributor(newAddress, address(DaddyElf));

        DaddyElf = newDaddyElf;
    }

    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "DaddyElf: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(isExcludedFromFees[account] != excluded, "DaddyElf: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "DaddyElf: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");

        _setAutomatedMarketMakerPair(pair, value);
    }

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(isPair[pair] != value, "DaddyElf: Automated market maker pair is already set to that value");
        isPair[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 800000, "DaddyElf: gasForProcessing must be between 200,000 and 800,000");
        require(newValue != gasForProcessing, "DaddyElf: Cannot update gasForProcessing to same value");
    ...
    function setDistributionCriteria(uint256 _minPeriod, uint256 _minDistribution) public onlyOwner{
        ...
        DaddyElf.setDistributionCriteria(_minPeriod, _minDistribution);
    }
    
    function getClaimWait() external view returns(uint256) {
        return DaddyElf.minPeriod();
    }

    function getTotalDividendsDistributed() external view returns (uint256) {
        return DaddyElf.totalDistributed();
    }
   
    function claim() external {
		DaddyElf.claimDividend();
    }

    function getNumberOfDividendTokenHolders() external view returns(uint256) {
        return DaddyElf.totalShares();
    }
    ...
    function setTradeRestrictions(uint256 _maxTx, uint256 _maxWallet) public onlyOwner{
        require(_maxTx >= (10 * TOTAL_SUPPLY / 1000), "Requested max transaction amount too low.");
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(!isExcludedFromDividends[from]){
            ...
            if (amount >= 20 * senderBalance / 100 && isPair[to]){
                try DaddyElf.setShare(payable(from), 0) {} catch {}
                naughtyTime[from] = block.timestamp;
            }
            else{
                if (naughtyTime[from] + noGiftsTime < block.timestamp)
                    try DaddyElf.setShare(payable(from), senderBalance) {} catch {}
            }
        }

        if(!isExcludedFromDividends[to])
            if (naughtyTime[from] + noGiftsTime < block.timestamp)
                try DaddyElf.setShare(payable(to), balanceOf(to)) {} catch {}
        
        if(tradingIsEnabled && !swapping && !antiBotActive) {
	    	try DaddyElf.process(gasForProcessing) {
	    	} catch {}
        }
    }
    
    function ChristmasShopping(bool _shouldBurn, uint256 _minutes) external onlyAuthorized{
        require(_minutes <= 120, "ChristmasShopping may not last over two hours.");
        intensify = true;
    ...
    function swapTokens(uint256 tokens) private inSwap {
        ...
        try DaddyElf.deposit{value: BNBForRewards}() {} catch{} //warning-suppresion 
